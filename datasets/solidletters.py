...
#Take the files in '.bin' format and match to filenames in '.txt' file and save to a list named "files"
def _get_filenames(root_dir, filelist):
    with open(str(root_dir / f"{filelist}"), "r") as f:
        file_list = [x.strip() for x in f.readlines()]

    files = list(x for x in root_dir.rglob(f"*.bin") if x.stem in file_list) #So .bin and .txt (train/test) must in same folder
    return files
...
class SolidLetters(BaseDataset):
    ...
    def __init__(self,
        ...
        if split in ("train", "val"):
            ...
            train_files, val_files = train_test_split(file_paths, test_size=0.2, random_state=42, stratify=labels)
            if split == "train":
